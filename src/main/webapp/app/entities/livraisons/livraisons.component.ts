...
export class LivraisonsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.livraisonsService
        ...
        .subscribe(
          (res: HttpResponse<ILivraisons[]>) => this.paginateLivraisons(res.body, res.headers),
  ...
  trackId(index: number, item: ILivraisons) {
    return item.id;
  }

  registerChangeInLivraisons() {
    this.eventSubscriber = this.eventManager.subscribe('livraisonsListModification', response => this.loadAll());
  }
  ...
  protected paginateLivraisons(data: ILivraisons[], headers: HttpHeaders) {
