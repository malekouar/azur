...
export class ServeurComponent implements OnInit, OnDestroy {
  currentAccount: any;
  serveurs: IServeur[];
  error: any;
  success: any;
  ...
  loadAll() {
    if (this.currentSearch) {
      this.serveurService
        ...
        .subscribe(
          (res: HttpResponse<IServeur[]>) => this.paginateServeurs(res.body, res.headers),
  ...
  registerChangeInServeurs() {
    this.eventSubscriber = this.eventManager.subscribe('serveurListModification', response => this.loadAll());
  }
